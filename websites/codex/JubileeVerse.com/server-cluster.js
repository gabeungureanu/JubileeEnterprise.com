/**
 * JubileeVerse - Clustered Server with Zero-Downtime Reload
 *
 * This wrapper provides:
 * - Multi-core clustering (uses all CPU cores)
 * - Zero-downtime reloads via SIGUSR2 signal
 * - Automatic worker restart on crash
 * - Graceful shutdown
 *
 * Usage:
 *   Start: node server-cluster.js
 *   Reload (zero-downtime): kill -SIGUSR2 <master-pid>
 *   Or on Windows: node reload-service.js
 */
...
if (cluster.isPrimary || cluster.isMaster) {
  ...
  // Listen for reload signal (SIGUSR2)
  process.on('SIGUSR2', () => {
    console.log('[Cluster] Received SIGUSR2 - starting zero-downtime reload');
    reloadWorkers();
  });

  // Also listen for custom message to trigger reload (for Windows)
  ...
  // Create a simple reload trigger file watcher (alternative for Windows)
  const fs = require('fs');
