...
/**
 * Get all Flywheel sections (Data, Intelligence, Execution)
 * GET /api/admin/flywheel/sections
 */
const getFlywheelSections = asyncHandler(async (req, res) => {
  const result = await database.query(`
    SELECT
      fcc.id,
      fcc.slug,
    ...
    GROUP BY fcc.id, fcc.slug, fcc.name, fcc.display_name, fcc.description,
             fcc.section_order, fcc.qdrant_prefix, fcc.icon, fcc.icon_color, fcc.is_active
...
/**
 * Get all Flywheel collections grouped by section
 * GET /api/admin/flywheel/collections
 */
const getFlywheelCollections = asyncHandler(async (req, res) => {
  // Get sections first
  ...
  // Get all collections
  const collectionsResult = await database.query(`
    SELECT
      fc.id,
      fc.slug,
      fc.name,
  ...
  const sections = sectionsResult.rows.map(section => ({
    ...section,
    collections: collectionsResult.rows.filter(c => c.category_id === section.id)
  }));

  res.json({
    success: true,
    sections,
    totalCollections: collectionsResult.rows.length
  });
});

/**
 * Get a single Flywheel collection by slug or ID
 * GET /api/admin/flywheel/collections/:idOrSlug
 */
const getFlywheelCollection = asyncHandler(async (req, res) => {
  const { idOrSlug } = req.params;
  const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(idOrSlug);
  ...
  const result = await database.query(`
    SELECT
      ...
      fcc.slug AS section_slug,
      fcc.section_order,
      fcc.icon AS section_icon,
      fcc.icon_color AS section_color
    FROM flywheel_collections fc
    JOIN flywheel_collection_categories fcc ON fc.category_id = fcc.id
    WHERE ${isUUID ? 'fc.id = $1' : 'fc.slug = $1'}
  `, [idOrSlug]);

  if (result.rows.length === 0) {
    return res.status(404).json({ success: false, message: 'Flywheel collection not found' });
  }
...
/**
 * Get roots (categories) for a specific Flywheel collection
 * GET /api/admin/flywheel/collections/:slug/roots
 *
 * Returns all root categories associated with a Flywheel collection.
 * Roots are the first-level categories that organize data within each collection.
 */
const getFlywheelCollectionRoots = asyncHandler(async (req, res) => {
  ...
  if (collectionResult.rows.length === 0) {
    return res.status(404).json({
      success: false,
      message: 'Flywheel collection not found'
    });
  }

  const collection = collectionResult.rows[0];

  // Get roots for this collection
  const rootsResult = await database.query(`
    SELECT
      fcc.id,
      fcc.slug,
      fcc.name,
    ...
    ORDER BY fcc.display_order, fcc.name
  `, [collection.id]);
...
module.exports = {
  ...
  // Flywheel Collections
  getFlywheelSections,
  getFlywheelCollections,
  getFlywheelCollection,
  getFlywheelCollectionRoots,
