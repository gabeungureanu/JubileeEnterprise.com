...
async function SearchUserPrompt(text) {
    try {
        activateThinking();
        await $.ajax({
            type: "POST",
            url: "/Home/GetJubileeGPTAudioNew",
            contenttype: "application/json;charset=utf-8",
            datatype: "json",
            async: false,
            data: { "userPrompt": text, "parentID": "" },
...
document.addEventListener('DOMContentLoaded', () => {
    audioManager = new AudioManager('audioPlayer', '/Audio/audioNew.mp3');
    //document.body.addEventListener('click', allowAudioAutoplayOnce, { once: true });
});
...
async function showResponse(responseText, replacedText, IsBuffer) {
    ...
    //const languageCode = await detectLanguages(removeHtmlTags(responseText));
    //if (('speechSynthesis' in window && languageCode === 'en')) {
    innerHtml += ' <div class="user-response"><div class="result-container ' + serialNumber + ' newresponse"><div class="askyeshua-icon"><img src="' + imgSrc + '" /></div><div class="result-wrapper" style="padding-left:10px;"><div class="result-title"> <p><strong>' + replacedText + '</strong></p><div class="speak-loud"><img src="/images/speaker.svg" class="start-speak" id="playBtn' + serialNumber + '" title="Speak Aloud" onclick="speakText(event,\'result' + serialNumber + '\')" /><img src="/images/play-stop.svg" class="stop-speak" id="stopBtn' + serialNumber + '" title="Stop" onclick="stopSpeaking(event)" /></div></div><div class="results" id="result' + serialNumber + '">' + responseText + '</div></div></div></div>';
    //} else {
...
function MakeAIReadyToRespond() {
    $.ajax({
        type: "get",
        url: "/Home/MakeAIReadyToRespond",
        contenttype: "application/json;charset=utf-8",
        datatype: "json",
