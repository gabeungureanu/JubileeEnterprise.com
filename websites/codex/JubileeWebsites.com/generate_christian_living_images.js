/**
 * Generate Images for Christian Living Category
 * Uses DALL-E 3 model
 */
...
async function getArticles() {
  const filePath = path.join(SITE_PATH, 'christian-living', 'web_articles.json');
  const data = JSON.parse(await fs.readFile(filePath, 'utf-8'));
  return data.articles;
}

async function generateImage(article, index, total) {
  const imagePrompt = `Create a professional, modern header image for a Faith-Based/Christian website article titled "${article.title}" in the category "Christian Living". The image should be clean, inspiring, and suitable for a news/blog website header banner. No text in the image. Professional photography style. The image should evoke themes of practical faith, daily Christian life, spiritual growth, and living purposefully. Warm, inviting colors. LANDSCAPE orientation.`;
...
async function main() {
  console.log('========================================');
  console.log('Generating Images for Christian Living');
  console.log('========================================');
